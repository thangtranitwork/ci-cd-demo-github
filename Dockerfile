...
# Biên dịch mã nguồn thành file chạy (Tắt CGO để file chạy độc lập trên các môi trường)
RUN CGO_ENABLED=0 GOOS=linux go build -o main .

# Bước 2 (Run Stage): Chuyển sang một bộ lọi cực nhẹ để vận hành
FROM alpine:latest

WORKDIR /root/

# Copy cái file 'main' đã biên dịch thành công từ bước 1 sang đây
